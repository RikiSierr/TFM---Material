---
title: "Algoritmo de Predicción - Porteros"
...
```{r}
columnas_cluster <- grep("Cluster", names(GK_merged), value = TRUE)
GK_merged_with_dummies <- dummy_cols(GK_merged, 
                                     select_columns = columnas_cluster, 
                                     remove_first_dummy = TRUE, 
                                     remove_selected_columns = TRUE)

# Crear nombres válidos para las columnas
valid_column_names <- make.names(names(GK_merged_with_dummies))
names(GK_merged_with_dummies) <- valid_column_names

year_threshold <- 2023

# Dividimos los datos en train (las 5 primeras temporadas) y test (temporada 2022 - 2023)
GK_merged_train <- subset(GK_merged_with_dummies, GK_merged_with_dummies$Season_End_Year < year_threshold)
GK_merged_test <- subset(GK_merged_with_dummies, GK_merged_with_dummies$Season_End_Year >= year_threshold)


# Definimos las variables respuestas (solo GK)
response_vars <- grep("_GK$", valid_column_names, value = TRUE)
response_eliminar  <- grep(c("Nation|Age"), response_vars, value = TRUE)
response_vars <- response_vars[!response_vars %in% response_eliminar]


# DefinimoS las variables predictoras (GK + Teams incluyendo clusters)
predictor_vars <- grep("_GK|_Teams", valid_column_names, value = TRUE)
...
calcular_estadisticas <- function(df, variables) {
  ...
  estadisticas <- lapply(variables, function(var) {
    media <- mean(df_numericas[[var]], na.rm = TRUE)
    varianza <- var(df_numericas[[var]], na.rm = TRUE)
    list(media = media, varianza = varianza)
  })
  names(estadisticas) <- variables
  return(estadisticas)
}
...
estadisticas_train <- calcular_estadisticas(GK_merged_train, predictor_vars)
...
for (response_var in response_vars) {
  ...
  cor_with_response <- sapply(GK_merged_train[predictors], 
                              function(x) cor(x, GK_merged_train[[response_var]], use = "complete.obs"))
  ...
  cluster_columns <- grep("Cluster", final_predictors, value = TRUE)

  # if (length(cluster_columns) > 0) {
  #   # Identificamos las columnas que no contienen "Cluster"
  #   non_cluster_columns <- setdiff(final_predictors, cluster_columns)
  #   
  #   # Escalar solo las columnas que no contienen "Cluster" en el dataframe de entrenamiento
  #   GK_merged_train[non_cluster_columns] <- scale(GK_merged_train[non_cluster_columns])
  #   
  #   # Escalar solo las columnas que no contienen "Cluster" en el dataframe de prueba
  #   GK_merged_test[non_cluster_columns] <- scale(GK_merged_test[non_cluster_columns])
  # } else {
  #   # Si no hay columnas "Cluster", escalar todas las columnas especificadas
  ...
  tryCatch({
    ...
    final_model <- lme(
      ...
      # correlation = corAR1(from = ~ Season_End_Year | Player),
    ...
    # final_model <- glmmTMB(
    #       formula = Min_Playing.Time_Generales_GK ~ Season_End_Year + (1 | Comp), 
    #      data = GK_merged_train,
    ...
    #       start = init_params
    #       )
    models_GK[[response_var]] <- final_model
  }, error = function(e) {
...
output_dir <- "C:/Users/rsier/Desktop/MASTERS/LaSalle - MUDS/TFM/Code/6. MODELOS/GK/"  # Cambia esto a tu ruta deseada
...
saveRDS(models_GK, file = paste(output_dir, "models_GK_list.rds"))
...
Seleccionamos los datos chorentes, de los que dispondriamos en un caso real, para la prediccion
```{r}
set.seed(410)
GK_subset <- subset(GK_merged_with_dummies, GK_merged_with_dummies$Season_End_Year == 2022)
GK_subset_muestra <- GK_subset[sample(nrow(GK_subset), 50), ]
GK_subset_muestra_4_predict <- GK_subset_muestra[, c("Player" , grep("_GK", valid_column_names, value = TRUE))]
GK_subset_muestra_4_predict$Age_GK <- GK_subset_muestra_4_predict$Age_GK + 1 


Teams_subset <- subset(GK_merged_with_dummies, GK_merged_with_dummies$Season_End_Year == 2022)
...
data_4_predict <- cbind(GK_subset_muestra_4_predict, Teams_subset_muestra_4_predict)
# data_4_predict_scale <- escalar_datos(df = data_4_predict, 
#                                       variables = colnames(data_4_predict)[c(4:19, 23:79)], 
...
```{r}
# Supongamos que tienes una lista de modelos llamada models_GK y un dataframe de prueba GK_merged_test
...
for (iter in 1:num_iter) {
  ...
  for (response_var in response_vars) {
    # response_var = response_vars[1]
    predictions_i <- simulate_predict(models_GK[[response_var]], data_4_predict)
...
# predictions_history$Min_Playing.Time_Generales_GK
...
output_dir <- "C:/Users/rsier/Desktop/MASTERS/LaSalle - MUDS/TFM/IMG/6. Prediccion/1. Variacion_Pred_times/POR/"  # Cambia esto a tu ruta deseada
